...
void Painter::renderDebugText(DebugBucket& bucket, const mat4 &matrix) {
    ...
    bucket.drawLines(*plainShader, config);

#ifndef GL_ES_VERSION_2_0
    // Draw line "end caps"
    MBGL_CHECK_ERROR(glPointSize(2));
    bucket.drawPoints(*plainShader, config);
#endif
...
void Painter::renderDebugFrame(const mat4 &matrix) {
    ...
    // draw tile outline
    tileBorderArray.bind(*plainShader, tileBorderBuffer, config, BUFFER_OFFSET(0));
    plainShader->u_color = {{ 1.0f, 0.0f, 0.0f, 1.0f }};
    lineWidth(4.0f * data.pixelRatio);
    MBGL_CHECK_ERROR(glDrawArrays(GL_LINE_STRIP, 0, static_cast<GLsizei>(tileBorderBuffer.index())));
}
